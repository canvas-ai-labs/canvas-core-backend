...
class MockCanvasLLMService:
    ...
    def summarize_syllabus(self, course_id: int) -> Dict[str, Any]:
        ...
        course = self.db.query(Course).filter(
            Course.canvas_course_id == course_id
        ).first()
    ...
    def analyze_assignment(self, assignment_id: int) -> Dict[str, Any]:
        ...
        assignment = self.db.query(Assignment).join(Course).filter(
    ...
    def generate_study_plan(self, user_id: int, days_ahead: int = 14) -> Dict[str, Any]:
        ...
        ).order_by(Assignment.due_at).limit(10).all()
        ...
        for day in range(min(7, days_ahead)):  # Generate plan for up to 7 days
            ...
            tasks = []
            for i, assignment in enumerate(upcoming_assignments[:2]):
                ...
                priority = "high" if (assignment.due_at - datetime.now(timezone.utc)).days <= 2 else "medium"
    ...
    def ask_question(self, user_id: int, question: str, context_course_id: Optional[int] = None) -> Dict[str, Any]:
        """Mock Q&A responses."""
