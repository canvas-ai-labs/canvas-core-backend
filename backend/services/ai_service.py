...
from backend.models import Course, Assignment, User, NotificationLog
...
class CanvasAIService:
    ...
    def get_upcoming_deadlines(self, user_id: int, days_ahead: int = 7) -> List[Dict[str, Any]]:
        ...
        upcoming_assignments = self.db.query(Assignment).join(Course).filter(
            and_(
        ...
        for assignment in upcoming_assignments:
            ...
            deadlines.append({
                ...
                "submission_types": assignment.submission_types.split(',') if assignment.submission_types else []
            })
    ...
    def get_overdue_assignments(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        overdue_assignments = self.db.query(Assignment).join(Course).filter(
            and_(
                Assignment.due_at.isnot(None),
                Assignment.due_at < current_time,
                Assignment.workflow_state != 'deleted'
    ...
    def get_course_workload_analysis(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        for course in courses:
            ...
            if assignment_count > 0:
                avg_days_until_due = sum(
                    (a.due_at - current_time).days for a in assignments
                ) / assignment_count
                
                # Calculate workload intensity
                intensity = "high" if assignment_count >= 5 or total_points >= 500 else \
                           "medium" if assignment_count >= 3 or total_points >= 200 else "low"
                ...
                courses_data.append({
                    ...
                    "upcoming_assignments": [
                        ...
                        } for a in assignments[:3]  # Show next 3 assignments
                    ]
                })
    ...
    def generate_study_recommendations(self, user_id: int) -> Dict[str, Any]:
        ...
        if total_upcoming > 0:
            ...
            if avg_days_ahead < 3:
                ...
                recommendations["time_management"]["message"] = "Focus on immediate deadlines with intensive daily sessions."
            elif avg_days_ahead < 7:
                recommendations["time_management"]["strategy"] = "balanced"
                recommendations["time_management"]["message"] = "Maintain steady progress across all assignments."
            else:
                recommendations["time_management"]["strategy"] = "planning"
                recommendations["time_management"]["message"] = "Great! You have time to plan and work systematically."
    ...
    def create_deadline_notification(self, user_id: int, assignment_id: int, notification_type: str = "deadline_reminder") -> NotificationLog:
        """Create a deadline notification for a user."""
        assignment = self.db.query(Assignment).filter(
            Assignment.canvas_assignment_id == assignment_id
        ).first()
